...
public:
    FrameCloudDebugNode() : nh_(), pnh_("~") {
        ...
        pnh_.param<std::string>("height_mode", height_mode_, "fixed");
        pnh_.param<double>("gate_center_z", gate_center_z_, flight_z_);
        pnh_.param<double>("crossing_z", crossing_z_, flight_z_);
        pnh_.param<bool>("use_detected_center_z", use_detected_center_z_, false);
        pnh_.param<bool>("goal_z_follows_center", goal_z_follows_center_, false);
        ...
        pnh_.param<double>("auto_height_top_z_max", auto_height_top_z_max_, 1.25);
...
private:
    ...
    void cloudCb(const sensor_msgs::PointCloud2ConstPtr& msg) {
        ...
        if (split_candidates.size() >= 2 &&
            ...
            if (detection.valid) {
                history_.clear();
                publishOutputs(detection, frame_id, msg->header.stamp);
    ...
    HeightEstimate candidateHeightEstimate(const GatePostCandidate& post,
        ...
        if (post.points > 0) {
            z_values.reserve(2);
            z_values.push_back(post.low_z);
            z_values.push_back(post.high_z);
        }

        HeightEstimate estimate;
        estimate.valid = post.points > 0 && post.z_span > 1e-3;
        estimate.source = source;
        estimate.points = post.points;
        estimate.low_z = post.low_z;
        estimate.high_z = post.high_z;
        estimate.span = post.z_span;
        const double bottom_z = autoHeightBottomZ(post.low_z);
        estimate.center_z = clampCenterZ(0.5 * (bottom_z + post.high_z) +
                                         detected_center_z_bias_);
        return estimate;
    }
    ...
    HeightEstimate candidateHeightEstimate(const GatePostCandidate& left_post,
                                           ...
                                           const std::string& source) const {
        HeightEstimate estimate;
        estimate.valid = (left_post.points + right_post.points) > 0;
        estimate.source = source;
        estimate.points = left_post.points + right_post.points;
        estimate.low_z = 0.5 * (left_post.low_z + right_post.low_z);
        estimate.high_z = 0.5 * (left_post.high_z + right_post.high_z);
        estimate.span = estimate.high_z - estimate.low_z;
        const double bottom_z = autoHeightBottomZ(estimate.low_z);
        estimate.center_z = clampCenterZ(0.5 * (bottom_z + estimate.high_z) +
                                         detected_center_z_bias_);
    ...
    HeightEstimate detectionCenterZ(const GatePostCandidate& left_post,
        ...
        return candidateHeightEstimate(left_post, right_post, "candidate_posts");
    }
    ...
    HeightEstimate detectionCenterZ(const GatePostCandidate& visible_post,
        ...
        return candidateHeightEstimate(visible_post, "candidate_visible_post");
    }
    ...
    void logFrame(const std::string& status,
        ...
        if (detection && detection->valid) {
            oss << " | pair gap=" << detection->gap
                ...
                << " center=(" << detection->center.x() << "," << detection->center.y() << "," << detection->center.z() << ")"
                << " pre=(" << detection->pre_goal.x() << "," << detection->pre_goal.y() << "," << detection->pre_goal.z() << ")"
                << " post=(" << detection->post_goal.x() << "," << detection->post_goal.y() << "," << detection->post_goal.z() << ")"
                << " yaw=" << detection->yaw;
        }
    ...
    void publishOutputs(const GateDetection& d, const std::string& frame_id, const ros::Time& stamp) {
    ...
    std::string height_mode_ = "fixed";
    double gate_center_z_ = 0.9;
    double crossing_z_ = 0.9;
    bool use_detected_center_z_ = false;
    bool goal_z_follows_center_ = false;
    ...
    double auto_height_top_z_max_ = 1.25;
